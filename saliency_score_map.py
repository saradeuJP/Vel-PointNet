...
class SphereAttack():
    ...
    def drop_points(self, pointclouds_pl, vel_pl, labels_pl, idx, sess):## BxNx3
        ...
        for i in range(self.k):
            ...
            indice_rank = np.argpartition(sphere_map,1)
            print(indice_rank)
            print(indice_rank.shape)
            
            # print(sphere_map.shape)
            # print(pointclouds_pl.shape)
            # print(vel_pl.shape)
            pts = PVGeo.points_to_poly_data(np.squeeze(pointclouds_pl))
            vel_pl = np.squeeze(vel_pl)
            pts['u'] = vel_pl[:,0].flatten(order="F") 
            pts['v'] = vel_pl[:,1].flatten(order="F") 
            pts['w'] = vel_pl[:,2].flatten(order="F") 
            pts['scores'] = indice_rank.flatten(order="F") 
            # pts['vel'] = vel.flatten(order="F") 

            pts.save(str(idx)+'saliency_map.vtk')
