...
parser.add_argument('--batch_size', type=int, default=13, help='Batch Size during training [default: 1]')
...
tmin = np.min([x1.min(),y1.min(),z1.min()])
tmax = np.max([x1.max(),y1.max(),z1.max()])
umin = np.min([u1.min(),v1.min(),w1.min()])
umax = np.max([u1.max(),v1.max(),w1.max()])
...
def eval_one_epoch(sess, ops, num_votes=1, topk=1):
    ...
    with open('test_label_external2_new.csv','ab') as f:               
              np.savetxt(f, label.reshape(-1,1))
    
    with open('test_pred_logits_external2_new.csv','ab') as f:               
              np.savetxt(f, (np.array(pred_val_1)).reshape(-1,1))
    
    with open('test_pred_external2_new.csv','ab') as f:               
